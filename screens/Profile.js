...
import { View, Text, TouchableOpacity, ScrollView, Modal, FlatList, Dimensions } from 'react-native';
...
const WASTE_COLORS = ['#F3B0A5', '#87B9C2', '#EC6039', '#FFB500', '#A8C3A4', '#4F6BB7', '#E7C665', '#699966'];
const EATEN_COLORS = ['#B39DBC', '#4EA8DE', '#90E0EF', '#DDA15E', '#98B4A6', '#5E60CE', '#D8C3A5', '#0077B6'];

const EMPTY_WASTED_FALLBACK = [{ value: 1, color: '#EAEAEA', label: 'No Waste', percentageText: '0%' }];
const EMPTY_EATEN_FALLBACK = [{ value: 1, color: '#EAEAEA', label: 'No Data Yet', percentageText: '0%' }];

export default function Profile() {
  const { activityLog = [] } = useContext(FridgeContext) || {};
  const [showDetailModal, setShowDetailModal] = useState(false);

  const stats = useMemo(() => calculateProfileStatistics(activityLog), [activityLog]);

  const hasLineData = stats.weeklyPoints.some(v => v > 0);
  const lineChartData = {
    labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    datasets: [{
      data: hasLineData ? stats.weeklyPoints : [0, 0, 0, 0, 0, 0, 0],
      strokeWidth: 3
    }]
  };

  const eatenDonutData = generateDonutData(stats.eatenCategoryCounts, stats.totalSavedItemsCount, EATEN_COLORS);
  const wastedDonutData = generateDonutData(stats.wastedCategoryCounts, stats.totalWastedItemsCount, WASTE_COLORS);
        ...
        <View style={styles.chartsRow}>
          <View style={styles.halfColumn}>
            ...
            <View style={styles.donutCardContainer}>
              <GiftedPieChart
                donut
                data={eatenDonutData || EMPTY_EATEN_FALLBACK}
                radius={45}
                innerRadius={25}
                showText={false}
                strokeWidth={3}
              ...
              <View style={styles.legendContainer}>
                {(eatenDonutData || EMPTY_EATEN_FALLBACK).map((item, idx) => (
          ...
          <View style={styles.halfColumn}>
            ...
            <View style={styles.donutCardContainer}>
              <GiftedPieChart
                donut
                data={wastedDonutData || EMPTY_WASTED_FALLBACK}
                radius={45}
                innerRadius={25}
                showText={false}
                strokeWidth={3}
              ...
              />
              <View style={styles.legendContainer}>
                {(wastedDonutData || EMPTY_WASTED_FALLBACK).map((item, idx) => (
                  <View key={idx} style={styles.legendRow}>
                    <View style={[styles.legendDot, { backgroundColor: item.color }]} />
                    <Text style={styles.legendLabel} numberOfLines={1}>
                      {item.percentageText} {item.label}
            ...
            renderItem={({ item, index }) => (
              <View style={styles.listItem}>
                <View style={styles.listItemLeft}>
                  <View style={[styles.colorDot, { backgroundColor: WASTE_COLORS[index % WASTE_COLORS.length] }]} />
...
const calculateProfileStatistics = (activityLog = []) => {
  ...
  activityLog.forEach(log => {
    ...
    if (logDate >= startOfWeek && logDate < endOfWeek) {
      if (action === 'consumed' || action === 'wasted') {
        const lineDayName = WEEKDAY_NAMES[logDate.getDay()];
